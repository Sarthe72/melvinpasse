const buildApplicationLetter=item=>{const proofs=item.analysis.evidence.map(proof=>`${proof.title} : ${proof.facts.join(" ; ")}`);const matches=item.analysis.matches.slice(0,4).join(", ")||"le pilotage opérationnel";return`Madame, Monsieur,
...
function letter(id){const item=getItem(id);if(!item)return dashboard();const content=buildApplicationLetter(item);layout(item.title,"Lettre de motivation · PDF",`<div class="actions"><button class="button" id="print-letter">Télécharger en PDF</button><a class="button secondary" href="#kit/${id}">Retour au kit</a></div><div class="letter-screen"><article class="letter-page"><header><strong>${esc(profile.identity.name)}</strong><span>${esc(profile.identity.email)} · ${esc(profile.identity.phone)}</span></header><div class="letter-recipient"><b>${esc(item.company)}</b><span>Candidature au poste de ${esc(item.title)}</span></div><div class="letter-body">${esc(content).replaceAll("\n","<br>")}</div></article></div>`);document.querySelector("#print-letter").onclick=()=>window.print();if(sessionStorage.getItem("cv-melvin-auto-pdf")===`letter/${id}`){sessionStorage.removeItem("cv-melvin-auto-pdf");printThenReturn(id)}}
